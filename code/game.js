...
function Player(pos) {
  this.pos = pos.plus(new Vector(1, -1.5));
...
function Bouncer(pos) {
  this.boPos = this.pos = pos.plus(new Vector(0.2, 0.1));
  this.size = new Vector(.25, .25);
  this.bounce = Math.random() * Math.PI * 2;
}
Bouncer.prototype.type = "bouncer";
...
function Enemy(pos) {
  this.enemyPos = this.pos = pos.plus(new Vector(1, 0));
  this.size = new Vector(1, .5);
  this.enemy = Math.random() * Math.PI * 3;
...
Flood.prototype.act = function(step, level) {
  var floPos = this.pos.plus(this.flood.times(step));
  if (!level.obstacleAt(floPos, this.size))
    this.pos = floPos;
  else if (this.repeatPos)
    this.pos = this.repeatPos;
  else
    this.flood = this.flood.times(-1);
};
...
var bounceSpeed = 10, bounceDist = 1;

var enemySpeed = 3, enemyDist = 7;

Enemy.prototype.act = function(step) {
  this.enemy += step * enemySpeed;
  var enemyPos = Math.sin(this.enemy) * enemyDist;
  this.pos = this.enemyPos.plus(new Vector(enemyPos, 0));
};
...
Bouncer.prototype.act = function(step) {
  this.bounce += step * bounceSpeed;
  var bouncePos = Math.sin(this.bounce) * bounceDist;
  this.pos = this.boPos.plus(new Vector(0, bouncePos));
};
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  } else if (type == "flood" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
